...
df_trials <- df %>%
  ...
  select(
    Subject,Key,Age,Gender,Block,Trial,Prime,primeCat,Target,targetCat,Response,RT,logRT,
    Correct,Accuracy,speedIncl,accIncl,ruleIncl,subjIncl,trialIncl,Inclusion
  )
...
df_trials %>%
  ...
  multiply_by(100) %>%
  signif(2) %>%
  paste0("% of trials from included participants were ultimately included, based on speed and accuracy.")
...
fit_multi <- df_trials %>%
  filter(Inclusion) %>%
  mixed(
    logRT ~ primeCat * targetCat + (1|Prime) + (1|Target) + (1|Subject),
    data = .,
    method='S'
  )
