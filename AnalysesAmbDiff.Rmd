...
lapply(
  c("plyr","dplyr","tidyr","ggplot2","readxl","readr","lme4","afex","lmerTest",
    "emmeans","purrr","magrittr","cowplot","stringr","rstan"),
  require,
  character.only = T
)
```
...
The command executed was: fast-dm.exe model_ks_congr_corr.ctl
Note that this command was executed within the fastDM folder, so that the data is correctly loaded.
The code below should do the same, but sadly does not seem to output the progress, making the slow process more difficult to overview.
...
Note, I could've, way easier, just imported the "parameter.lst" text document, which is formatted as my 

```{r import, options}

RAWDIFF <- "fastDM/Results/sv0" %>%
...
dfDiff <- RAWDIFF %>%
  spread(V1,V2) %>%
  select(-c("fitindex","method","penalty","precision")) %>%
  gather(
    "key",
    "Value",
    matches("_\\d")
...
Predicted data through simulation within fast-dm?

```{r Quantile Probability Plots, options}

# 1 graph / participant (or for aggregated data)
# Horizontal: Response probability for different conditions
# Vertical: RT quantiles
...
# Vertical: Cumulative Probability


```

```{r Scatterplots, options}

# 1 graph / 1 statistic across participants and conditions
# Horizontal: Data
# Vertical: Prediction
...
# Comparison of observed and predicted CDFs (for each model)
# Biased: predicted distribution is fitted to empirical data -> too liberal
...
# Comparison of observed vs. predicted number of observations in RT bins (for each model)
# df = C * 11 - P
# Problems with low error rates
...
# shell("cd fastDM; .\\construct-samples.exe -a xxx -v xxx -t xxx -d xxx -Z -V -T -p -n 192 -N 1000 -r -o "%d.sim" ",shell = "PowerShell", intern=TRUE)
...
You could do this via group_by(Parameter) %>% do(), but I can't get to save the mixed objects this way, do expects dataframes of some sort.
The lapply version does seem to work :)
