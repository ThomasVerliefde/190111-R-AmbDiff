...
df %>%
  ...
  multiply_by(100) %>%
  signif(3) %>%
  paste0("% of trials from included participants were responed to too slowly (>3000ms)")
...
df %>%
  ...
  multiply_by(100) %>%
  signif(3) %>%
  paste0("% of trials from included participants were responed to too quickly (<300ms)")
...
emmCongrFit %>%
  contrast(method = "trt.vs.ctrl",ref=3,adjust="none")
...
```
 ...
 contrast                 estimate      SE  df z.ratio p.value
 Ambivalent - Incongruent -0.00419 0.00538 Inf -0.779  0.4357 
 Congruent - Incongruent  -0.02493 0.00504 Inf -4.946  <.0001 
 Neutral - Incongruent    -0.01499 0.00537 Inf -2.791  0.0053 

 contrast                 estimate      SE  df z.ratio p.value
 Congruent - Ambivalent   -0.02074 0.00534 Inf -3.884  0.0001 
 Incongruent - Ambivalent  0.00419 0.00538 Inf  0.779  0.4357 
 Neutral - Ambivalent     -0.01080 0.00545 Inf -1.982  0.0474 
...
Fast-dm expects the data to be in a certain format.
Crucially, reaction times should be in seconds.
...
load Data\\dat*.txt
save Results\\Full\\*.par
log parameterFull.lst" %>%
  cat(
    file = "fastDM/model_ks_congr_corrFull.ctl"
  )
...
dfDiff01 <- "fastDM/parameterFull.lst" %>%
...
Explanations:
  a, sv, szr, st0 - parameters diffusion, constant over conditions
...
Penalty is 0 for all participants, which is good.
Fit is more spread, although it is less clear what this means, since p-values are multiplied across multiple conditions, and value is too liberal, since the predictions are fit to the data themselves (specific to the KS method fit test).
...
dfDiff01 %>%
  ...
  select(
    ...
    ddply(
      ...
      assign("covDiff01",.,envir = globalenv())
  } %>%
  dlply(
    ~ Congruency,
    function(x) select(x,-Congruency) %>% colMeans %>% as.matrix %>% t
  ) %>%
  assign("meansDiff01",.,envir=globalenv())

set.seed(20190111)
rmvPars01 <- c("Ambivalent","Congruent","Incongruent","Neutral") %>%
  set_names(
    llply(
      .,
      function(x) {
        rmvnorm(
          n = 250,
          mean = meansDiff01 %>% extract2(x),
          sigma = covDiff01 %>% extract2(x),
          method = "chol"
        ) %>%
          set_colnames(
            meansDiff01 %>%
...
rmvPars01 %>% {
        ...
        extract2(.,x) %>%
          alply(
            ...
            function(y) {
              y %$%
                sprintf(
                  "construct-samples.exe -a %f -Z %f -V %f -T %f -d %f -t %f -v %f -p %f -N 1 -n 48 -r -o %s",
                  a,Z=szr,V=sv,T=st0,d,t=t0,v,p=3,paste0("Sim/Full/",x)
                )
            }
          ) %>% {
            llply(
  ...
  write_lines(
    path = "fastDM/fullSim.cmd"
  )

```

This batch file was then executed via cmd in the fastDM folder (command: fullSim.cmd)

```{r sims, options}

simDiff01 <- c("Ambivalent","Congruent","Incongruent","Neutral") %>% 
  ldply(
    function(x) {
      list.files("fastDM/Sim/Full/",x,full.names=TRUE) %>%
        ldply(
          function(y) {
            read.table(y) %>%
              transmute(
                Subject = gsub("\\D","",y) %>% as.numeric,
                Response = V1,
                RT = V2,
                )
          }
  ...
  d_ply(
    ...
    function(x) {
      write(
        ...
        paste0("fastDM/Sim/Full/Data/dat",x$Subject %>% unique,".txt")
      )
      write.table(
        x %>% select(-Subject),
        paste0("fastDM/Sim/Full/Data/dat",x$Subject %>% unique,".txt"),
...
# response is correct/false (1/0)
load Sim\\Full\\Data\\dat*.txt
save Sim\\Full\\Results*.par
log parameterFullSim.lst" %>%
  cat(
    file = "fastDM/model_ks_congr_corrFullSim.ctl"
  )
...
I've opted to only use the data from full simulated participants (N = 202 out of 250), since (I assume) some parameter values (such as negative a values) are not functional within the diffusion framework, but were nevertheless computed randomly by the multivariate random distribution.

As with our real data, diffusion model analyses were computed via cmd (fast-dm.exe model_ks_congr_corrFullSim.ctl)

The 5% quantile of the distribution of fit indices is then used as critical value to assess fit of empirical results

```{r fit-index, options}

dfDiff01Sim <- "fastDM/parameterFullSim.lst" %>%
  read.table(header=TRUE) %>%
  gather("key","Value",matches("_")) %>%
  separate(key,c("Parameter","Congruency"),sep="_") %>%
  mutate(
...
dfDiff01Sim %>%
  ...
  geom_vline(aes(xintercept=.05),colour="red")
...
dfDiff01 %>%
  ...
  ggplot(aes(x = fit)) +
  geom_histogram(binwidth = .01) +
  geom_vline(aes(xintercept=critFit),colour="red")

```

Since our comparison does not show any subjects below the critical fit boundary, we should be able to assume that our models with somewhat decently.
Note however, that the distribution of fit looks quite uniform for the simulated data.
This is better for our real data, although we have one quite low "outlier", which we reasonably should exclude.
...
Congruent - Ambivalent   -0.00994 0.00341 225 -2.917  0.0039

contrast                estimate      SE  df t.ratio p.value
Congruent - Incongruent  -0.0118 0.00341 225 -3.468  0.0006 

#############
#### Graphs
#############

```{r diff graphs, options}
...
#   )
# ) %>%
#   mutate(Parameter = Parameter %>% factor(levels = c("d","v","t0"))) %>%
#   ggplot(aes(x = Congruency, y = emmean, fill = Congruency, ymin = lower.CL, ymax = upper.CL)) +
#   geom_col(position=position_dodge2()) +
#   geom_errorbar(position = position_dodge2(),colour="black") +
#   facet_wrap(~Parameter,strip.position = "bottom",scales = "free_y") +
#   labs(y = "Estimated Means", x = "Parameter") +
#   theme(
#     strip.placement = "outside",
#     strip.background = element_blank(),
...
Fit01_d %>%
  ...
  as_tibble %>%
  mutate(Parameter = "d") %>%
  ggplot(aes(x = Parameter, y = emmean, fill = Congruency, ymin = lower.CL, ymax = upper.CL)) +
  geom_col(position=position_dodge2()) +
  geom_errorbar(position = position_dodge2(padding = .6),colour="black",size=.5) +
...
```{r congruency contrasts, options}
